    ...
    if FB_ACCESS_TOKEN is None:
        ...
        if "--fb-token" in sys.argv:
            ...
            if token_index < len(sys.argv):
                ...
                raise Exception(
                    "--fb-token argument provided but no token value followed it"
                )
        else:
            raise Exception(
                "Facebook token must be provided via '--fb-token' command line argument"
            )
...
def _fetch_edge(parent_id: str, edge_name: str, **kwargs) -> Dict:
    ...
    if edge_name == "activities":
        ...
        if time_range:
            ...
            if since:
                time_params["since"] = since
            if until:
                time_params["until"] = until
...
def get_adsets_by_ids(
    ...
    access_token = _get_fb_access_token()
    url = f"{FB_GRAPH_URL}/"
    params = {"access_token": access_token, "ids": ",".join(adset_ids)}
...
def get_campaigns_by_adaccount(
    ...
    if buyer_guarantee_agreement_status:
        params["buyer_guarantee_agreement_status"] = json.dumps(
            buyer_guarantee_agreement_status
        )
